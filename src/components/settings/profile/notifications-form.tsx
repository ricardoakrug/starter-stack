...
export default function NotificationsForm() {
  ...
  useEffect(() => {
    if (data && !form.formState.isDirty) {
      form.reset({
        emailEnabled: data.settings?.[0]?.emailEnabled ?? true,
        emailFrequency:
          (data.settings?.[0]?.emailFrequency as EmailFrequency) || "daily",
        emailAccountUpdates: data.settings?.[0]?.emailAccountUpdates ?? true,
        emailSecurityAlerts: data.settings?.[0]?.emailSecurityAlerts ?? true,
        emailMarketing: data.settings?.[0]?.emailMarketing ?? true,
        pushEnabled: data.settings?.[0]?.pushEnabled ?? true,
        pushAccountUpdates: data.settings?.[0]?.pushAccountUpdates ?? true,
        pushSecurityAlerts: data.settings?.[0]?.pushSecurityAlerts ?? true,
        pushMarketing: data.settings?.[0]?.pushMarketing ?? true,
        locale: data.settings?.[0]?.locale || "en",
        language: data.settings?.[0]?.language || "en",
      });
    }
  }, [data, form]);
